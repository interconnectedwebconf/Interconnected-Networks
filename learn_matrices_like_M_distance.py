...
class InterpretableTensorClassifier(nn.Module):
    def __init__(self, K, cp_rank, hidden=32):
        ...
        # Learnable matrices for interlink layer
        # For Z1_inter = appro_ic_1 @ S1 @ Z1_inter_reconstruct @ S2 @ appro_sir_2.T
        self.S1 = nn.Parameter(torch.eye(K, K))
        self.S2 = nn.Parameter(torch.eye(K, K))
        # For Z2_inter = appro_sir_1 @ S3 @ Z2_inter_reconstruct @ S4 @ appro_ic_2.T
        self.S3 = nn.Parameter(torch.eye(K, K))
        self.S4 = nn.Parameter(torch.eye(K, K))
    ...
    def forward(self, tensor_input, seed_ic, seed_sir, seed_ic_conv, seed_sir_conv, scalars):
        ...
        # # from tensor_input, extract appro_ic_1, appro_ic_2, appro_sir_1, appro_sir_2, Z1_inter_reconstruct, Z2_inter_reconstruct
        # appro_ic_1 = tensor_input[:, 0]
        # appro_ic_2 = tensor_input[:, 1]
        # appro_sir_1 = tensor_input[:, 2]
        # appro_sir_2 = tensor_input[:, 3]
        # Z1_inter_reconstruct = tensor_input[:, 4]
        # Z2_inter_reconstruct = tensor_input[:, 5]
        ...
        for i in range(B):
            ...
            # Reconstruct interlink layer with learnable matrices
            # Z1_inter = appro_ic_1 @ S1 @ Z1_inter_reconstruct @ S2 @ appro_sir_2.T
            
            Z1_inter = appro_ic_1 @ self.S1 @ Z1_inter_reconstruct @ self.S2 @ appro_sir_2.T
            # Z2_inter = appro_sir_1 @ S3 @ Z2_inter_reconstruct @ S4 @ appro_ic_2.T
            Z2_inter = appro_sir_1 @ self.S3 @ Z2_inter_reconstruct @ self.S4 @ appro_ic_2.T
            ...
            # Use updated_tensor_input for further operations
            cp_result = parafac(updated_tensor_input[i], rank=self.cp_rank, init='svd', n_iter_max=10)
...
with open(f'all_tensor_input_learning_M_distance.pkl', 'wb') as f:
...
with open('all_tensor_input_learning_M_distance.pkl', 'rb') as f:
